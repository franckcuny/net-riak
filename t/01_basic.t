...
my $bucket_name  = "test_$$";
my $bucket_multi = "test_multibucket_$$";

my @transports = qw(PBC REST);

foreach my $transport (@transports) {
    diag "Test with transport set to $transport";

    ok my $client = Net::Riak->new( transport => $transport ), "client created";
    ok $client->is_alive, "riak is alive";

    ok my $bucket = $client->bucket($bucket_name), "got test bucket";

    my $content = [ int( rand(100) ) ];

    my $key = "foo_" . time();
    ok my $obj = $bucket->new_object( $key, $content ),
      "created a new riak object '$key'";
    ok $obj->store, "store object $key";

    if ( $transport eq 'REST' ) {
        is $obj->status, 200, 'valid status';
    }

    is $obj->key,         $key,     'valid key';
    is_deeply $obj->data, $content, 'valid content';

    my $missing_object = $bucket->get("missing");
    ok !$missing_object->data, 'no data';

    $key = "foo_" . time();
    $obj = $bucket->new_object( $key, $content );
    ok $obj->store, "object '$key' is stored";
    $obj = $bucket->get($key);
    ok $obj->exists, "object '$key' exists";
    $obj->delete;
    $obj->load;
